...
DEFAULTS = {
    ...
    "merge_gate": {
        "redraft_cap": 3,
        "draft_batch_max": 10,           # max eligibility flags per draft task
        "defer_growth_factor": 0.20,     # counter growth past defer_watermark
                                          # required before a Defer-dispositioned
                                          # flag re-surfaces (0.20 = 20%)
    },
}
...
_ENV_SEEDS = {
    ...
    ("merge_gate", "draft_batch_max"):      ("HONE_DRAFT_BATCH_MAX", int),
    ("merge_gate", "defer_growth_factor"):  ("HONE_DEFER_GROWTH_FACTOR", float),
}
...
FIELDS = [
    ...
    ("merge_gate", "draft_batch_max", "Draft-task batch max",
     "eligibility flags per draft task", "int"),
    ("merge_gate", "defer_growth_factor", "Defer growth factor",
     "fraction of counter growth past defer_watermark required to re-surface",
     "float01"),
]
...
class RuntimeConfig:
    ...
    @property
    def draft_batch_max(self):
        return self._data["merge_gate"]["draft_batch_max"]

    @property
    def defer_growth_factor(self):
    ...
    for group, key, label, _unit, kind in FIELDS:
        ...
        if kind == "float01":
            try:
                ...
                errors[name] = f"{label} must be a number between 0 and 1"
                continue
            if not 0.0 <= value <= 1.0:
                errors[name] = f"{label} must be between 0 and 1"
            else:
